...
test('list rehydrates durable rows in stable order without witness authority fields', async () => {
  const fixture = transportFixture([
    {
      id: 'msg-2',
      circle_id: 'circle-a',
  ...
  const messages = await createConversationGateway(fixture.transport).list('circle-a');
...
test('blank, oversized, and cross-circle messages fail closed', async () => {
  const fixture = transportFixture();
  const gateway = createConversationGateway(fixture.transport);
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof ConversationContractError && error.code === 'EMPTY_MESSAGE',
  );
  await assert.rejects(
    () => gateway.send('circle-a', 'x'.repeat(4001)),
    (error: unknown) =>
      error instanceof ConversationContractError && error.code === 'MESSAGE_TOO_LONG',
  );
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof ConversationContractError && error.code === 'CROSS_CIRCLE_ROW',
  );
});

test('realtime callback rejects a row from another circle', () => {
  const fixture = transportFixture();
  const gateway = createConversationGateway(fixture.transport);
