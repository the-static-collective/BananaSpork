...
export interface ConversationTransport {
  ...
  subscribe(circleId: string, handler: (row: CircleMessageRow) => void): () => void;
}
...
function requireCircleId(circleId: string): string {
  ...
  if (!normalized) throw new ConversationContractError('EMPTY_CIRCLE_ID');
  return normalized;
}
...
export function createConversationGateway(transport: ConversationTransport): ConversationGateway {
  return {
    async list(circleId) {
    ...
    async send(circleId, body) {
      ...
      if (!normalizedBody) throw new ConversationContractError('EMPTY_MESSAGE');
      if (normalizedBody.length > MAX_MESSAGE_LENGTH) {
        throw new ConversationContractError('MESSAGE_TOO_LONG');
      }

      // Sender identity is not accepted from the browser. The authenticated
      // database constitution derives and checks sender_user_id.
